...
echo "
 ...
 | the CENT Team CANNOT assist you in repairing  |::
...
echo "" && echo 'Added user "cent"' && echo ""
sleep 1
...
masternode=1
addnode=182.206.23.95
addnode=69.10.50.56
addnode=162.35.173.254
addnode=163.245.218.219 


EOL
chmod 0600 $USERHOME/.cent/cent.conf
chown -R $USER:$USER $USERHOME/.cent

sleep 1

cat > /etc/systemd/system/cent.service << EOL
[Unit]
Description=centd
...
EOL
sudo systemctl enable cent.service
sudo systemctl start cent.service
